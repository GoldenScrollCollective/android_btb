...
class EditRecordActivity : AppCompatActivity(), OnItemClickListener {
    ...
    private fun appendCommonData() {
        ...
        mBinding.txtUser.text = eventModel!!.blessing_by_user_id!!.name



        dateOfEvent = eventModel?.blessing_complete.toString()
        mBinding.tvDateSelect.text = getDate(eventModel!!.blessing_complete!!)
        mBinding.txtTitle.text = eventModel!!.blessing_sent
        mBinding.txtNotes.text = eventModel!!.blessing_notes
        mBinding.etPrice.setText(eventModel!!.blessing_value)
    ...
    private fun setDataToUi(text: Any) {
        when (selectedType) {
            ...
            SelectedRecord.SELECT_USER -> {
                ...
                mBinding.txtUser.text = (text as UserListModel).name
            }
