...
import org.lemonadestand.btb.databinding.ActivityAddReminderBinding
...
class AddReminderActivity : AppCompatActivity(), OnItemClickListener {
	lateinit var mBinding: ActivityAddReminderBinding
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		mBinding = ActivityAddReminderBinding.inflate(layoutInflater)
	...
	private fun getUser() {
		selectedUser = Utils.getUser(this)
	}
	...
	private fun handleUIChanges() {
		...
		mBinding.tvTeamMember.text = selectedUser?.name
	...
	private fun checkValidation() {
		...
		if (eventFor == "Team") {
			resource = if (selectedUserListModel == null) {
				selectedUser!!.uniqueId
			} else {
				selectedUserListModel!!.uniq_id
			}
		...
		viewModel.addReminder(requestModel)
	...
	private fun showBottomSheetContact() {
		if (bottomSheetFragmentContact == null) {
			...
			bottomSheetFragmentContact!!.show(
				supportFragmentManager,
				bottomSheetFragmentContact!!.tag
			)
		}
