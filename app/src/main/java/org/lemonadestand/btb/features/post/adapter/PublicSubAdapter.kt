...
class PublicSubAdapter(
	...
	override fun onBindViewHolder(holder: ViewHolder, position: Int) {
		...
		if (data.type != null) {
			...
			holder.tvTitle.text = data.user.name + " +" + data.users.size            //Add Uses
			...
			holder.tvDesc.text = buildString {
				...
				append(" · ")
				append(data.modifiedAt()?.let { it.ago() })
			}
		} else {
			//Post
		...
		holder.tvComment.text =
			HtmlCompat.fromHtml(data.html, HtmlCompat.FROM_HTML_MODE_LEGACY).trim()
...
//            }
		...
		val commentsRecyclerViewAdapter =
			PostCommentsRecyclerViewAdapter(position, onItemClick)   // fixed
