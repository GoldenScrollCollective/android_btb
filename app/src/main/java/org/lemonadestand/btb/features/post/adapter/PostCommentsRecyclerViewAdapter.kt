...
class PostCommentsRecyclerViewAdapter(
...
) : BaseRecyclerViewAdapter<PostModel>(R.layout.layout_post_comment_list_item, true) {
	//    private var onPostItemClick: OnItemClickListener? = null
	fun setPostItemClick(listener: OnItemClickListener) {
		onPostItemClick = listener
	}
	...
	override fun bindView(holder: ViewHolder, item: PostModel, position: Int) {
		...
		with(holder.itemView) {
			...
			item.modifiedAt()?.let { agoView.text = it.ago() }

			val btnLike = findViewById<TextView>(R.id.btnLike)
			btnLike.setOnSingleClickListener {
				val view: View =
					LayoutInflater.from(context).inflate(R.layout.custom_like_menu, null)
				...
				like.setOnClickListener {
					...
					onPostItemClick?.onItemClicked(
						LikeBodyModel(
							metaName = "like", metaValue = "like", byUserId = Utils.getData(
								context,
								Utils.UID
							), uniqueId = item.uniq_id
						), position,
				...
				love.setOnClickListener {
					...
					onPostItemClick?.onItemClicked(
						LikeBodyModel(
							metaName = "like", metaValue = "love", byUserId = Utils.getData(
								context,
								Utils.UID
							), uniqueId = item.uniq_id
						), position,
				...
				awesome.setOnClickListener {
					...
					onPostItemClick?.onItemClicked(
						LikeBodyModel(
							metaName = "like", metaValue = "awesome", byUserId = Utils.getData(
								context,
								Utils.UID
							), uniqueId = item.uniq_id
						), position,
				...
				thanks.setOnClickListener {
					...
					onPostItemClick?.onItemClicked(
						LikeBodyModel(
							metaName = "like", metaValue = "thanks", byUserId = Utils.getData(
								context,
								Utils.UID
							), uniqueId = item.uniq_id
						), position,
				...
				haha.setOnClickListener {
					...
					onPostItemClick?.onItemClicked(
						LikeBodyModel(
							metaName = "like", metaValue = "haha", byUserId = Utils.getData(
								context,
								Utils.UID
							), uniqueId = item.uniq_id
						), position,
...
//                    // Handle the confirmation with the input message
						...
						val requestBody = AddCommentBody(
							uniq_id = "",
							resource = "user/${PublicFragment.currentUser!!.uniqueId}",
							html = message,
							created = "",
...
//                }
			...
			val repliesRecyclerViewAdapter =
				PostCommentsRecyclerViewAdapter(position, onPostItemClick)
